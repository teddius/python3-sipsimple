    ...
    def _CH_register(self, command):
        ...
        try:
            ...
            if self.account.sip.outbound_proxy is not None and self.account.sip.outbound_proxy.transport in settings.sip.transport_list:
                uri = SIPURI(
                    host=self.account.sip.outbound_proxy.host,
                    port=self.account.sip.outbound_proxy.port,
                    parameters={
                        'transport': self.account.sip.outbound_proxy.transport
                    }
                )
            else:
                # uri = SIPURI(host=self.account.id.domain) # NOTE: old code
                uri = self.account.uri
            lookup = DNSLookup()
            try:
                routes = lookup.lookup_sip_proxy(uri, settings.sip.transport_list,
            ...
            for route in routes:
                ...
                if remaining_time > 0:
                    ...
                    try:
                        self._registration.register(contact_header, route_header,
                                                    timeout=limit(remaining_time, min=1, max=10))
                    except SIPCoreError:
                    ...
                    try:
                        while True:
                            ...
                            if notification.name == 'SIPRegistrationDidEnd':
                                raise RegistrationError('Registration expired', retry_after=int(random.uniform(60,
                                                                                                               120)))  # registration expired while we were trying to re-register
                    except SIPRegistrationDidFail as e:
                        notification_data = NotificationData(code=e.data.code, reason=e.data.reason,
                                                             registration=self._registration, registrar=route)
                        ...
                        if e.data.code == 401:
                            ...
                            raise RegistrationError('Interval too short', retry_after=int(random.uniform(60, 120)),
                        ...
                        notification_data = NotificationData(code=notification.data.code,
                        ...
                        # Save GRUU
                        try:
                            header = next(header for header in notification.data.contact_header_list if
                                          header.parameters.get('+sip.instance', '').strip(
            ...
            notification_center.post_notification('SIPAccountRegistrationDidFail', sender=self.account,
                                                  data=NotificationData(error=e.error, retry_after=e.retry_after))
            ...
            if registered:
                ...
                try:
                    ...
                    notification_center.post_notification('SIPAccountRegistrationDidNotEnd', sender=self.account,
                                                          data=NotificationData(code=e.data.code, reason=e.data.reason,
    ...
    def _NH_CFGSettingsObjectDidChange(self, notification):
        ...
        if 'enabled' in notification.data.modified:
            ...
            if self.account.sip.register:
                ...
                self.deactivate()
        elif self.active and {'__id__', 'auth.password', 'auth.username', 'nat_traversal.use_ice', 'sip.outbound_proxy',
                              'sip.transport_list', 'sip.register_interval'}.intersection(notification.data.modified):
            self._command_channel.send(Command('unregister'))
            self._command_channel.send(Command('register'))
